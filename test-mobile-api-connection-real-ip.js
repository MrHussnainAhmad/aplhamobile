...
// Test the mobile app's API configuration with real IP
async function testMobileApiConnectionRealIP() {
  try {
    console.log('🧪 Testing Mobile App API Connection with Real IP...\n');

    // Test 1: Check if backend is accessible
    console.log('1️⃣ Testing backend connectivity...');
    const healthCheck = await axios.get('http://192.168.3.51:5000/api/admin/stats');
    ...
    try {
      await axios.post('http://192.168.3.51:5000/api/teacher/login', {
    ...
    try {
      await axios.post('http://192.168.3.51:5000/api/student/login', {
    ...
    const adminLoginResponse = await axios.post('http://192.168.3.51:5000/api/admin/login', {
    ...
    const configResponse = await axios.get('http://192.168.3.51:5000/api/admin/app-config');
    ...
    console.log('   3. The app should now connect to 192.168.3.51:5000');
    ...
    if (error.response) {
      ...
      console.log('   3. Check if your computer\'s firewall is blocking the connection');
    } else {
