...
const AttendanceDetailScreen = () => {
  ...
  const { type, id, name, classId, className, month } = route.params;
  ...
  const loadAttendanceRecord = async () => {
    try {
      setLoading(true);
      const response = await attendanceAPI.getAttendanceRecord(type, id, currentMonth);
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        <View style={styles.infoCard}>
          ...
          <Text style={styles.typeText}>{type === 'student' ? 'Student' : 'Teacher'}</Text>
        </View>
