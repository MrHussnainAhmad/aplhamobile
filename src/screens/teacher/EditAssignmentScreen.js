...
const EditAssignmentScreen = ({ navigation, route }) => {
  const { assignment } = route.params || {};
  
  // Check if assignment exists, if not navigate back
  useEffect(() => {
    if (!assignment) {
      Alert.alert('Error', 'No assignment data found. Please try again.');
      navigation.goBack();
      return;
    }
  ...
  // Form data - initialize with assignment data
  ...
  const [existingImages, setExistingImages] = useState(assignment?.images || []);
  ...
  const updateAssignment = async () => {
    ...
    try {
      ...
      const response = await teacherAPI.updateAssignment(assignment._id, formData);
      ...
      Alert.alert('Error', error.response?.data?.message || 'Failed to update assignment');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.formSection}>
          ...
          {existingImages.length > 0 && (
            <View style={styles.filesSection}>
              <Text style={styles.filesTitle}>Current Images ({existingImages.length})</Text>
              {existingImages.map((image, index) => (
                <View key={`existing-image-${index}`} style={styles.fileItem}>
                  <Image source={{ uri: image }} style={styles.imagePreview} />
                  <View style={styles.fileInfo}>
                    <Text style={styles.fileName}>Existing Image {index + 1}</Text>
                  </View>
                  <TouchableOpacity
          ...
          {images.length > 0 && (
            <View style={styles.filesSection}>
              <Text style={styles.filesTitle}>New Images ({images.length})</Text>
              {images.map((image, index) => (
                <View key={`new-image-${index}`} style={styles.fileItem}>
                  <Image source={{ uri: image.uri }} style={styles.imagePreview} />
                  <View style={styles.fileInfo}>
                    <Text style={styles.fileName}>New Image {index + 1}</Text>
                  </View>
                  <TouchableOpacity
                    style={styles.removeButton}
          ...
          {existingAttachments.length > 0 && (
            <View style={styles.filesSection}>
              <Text style={styles.filesTitle}>Current Files ({existingAttachments.length})</Text>
              {existingAttachments.map((file, index) => (
                <View key={`existing-file-${index}`} style={styles.fileItem}>
                  <Ionicons name="document-outline" size={24} color="#27AE60" />
                  <View style={styles.fileInfo}>
                    <Text style={styles.fileName}>{file.fileName || `File ${index + 1}`}</Text>
                    <Text style={styles.fileSize}>Existing file</Text>
                  </View>
                  <TouchableOpacity
                    style={styles.removeButton}
          ...
          {attachments.length > 0 && (
            <View style={styles.filesSection}>
              <Text style={styles.filesTitle}>New Files ({attachments.length})</Text>
              {attachments.map((file, index) => (
                <View key={`new-file-${index}`} style={styles.fileItem}>
                  <Ionicons name="document-outline" size={24} color="#27AE60" />
                  <View style={styles.fileInfo}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.updateButton, (loading || subjects.length === 0) && styles.disabledButton]}
        ...
        >
          {loading ? (
            ...
            <Text style={styles.updateButtonText}>No Subjects Available</Text>
          ) : (
            <>
